...
3D reconstruction is the process of estimating the shape, structure, and sometimes appearance of a real scene or object in three dimensions. The reconstructed output can take several forms, including a sparse point cloud, a dense point cloud, a polygon mesh, a textured mesh, a voxel grid, or a neural rendering representation. The appropriate representation depends on the purpose of the system. A sparse point cloud is useful for camera pose estimation and geometric evidence. A dense point cloud is useful for visual inspection and measurement. A mesh is useful for editing, animation, fabrication, or import into 3D applications. Gaussian Splatting is useful for novel-view visualization and photorealistic scene rendering.

Reconstruction methods are usually divided into active and passive approaches. Active methods use a device that emits a signal, such as laser light, structured light, infrared patterns, or LiDAR pulses. Passive methods rely on external illumination and infer geometry from images. S.P.E.C.T.R.A belongs mainly to the passive image-based category because it reconstructs scenes from smartphone images. However, it also borrows ideas from learned depth estimation, where a neural model predicts dense depth from single RGB images.

| Method Type | Examples | Main Strength | Main Limitation | Relevance to S.P.E.C.T.R.A |
|---|---|---|---|---|
| Active reconstruction | LiDAR, structured light, depth cameras | Can produce direct depth measurements | Requires specialized hardware | Used as a comparison point, not used directly |
| Passive reconstruction | Photogrammetry, SfM, stereo, MVS | Works with ordinary cameras | Sensitive to overlap, texture, blur, and calibration | Main project category |
| Learned depth reconstruction | MiDaS, Depth Anything V2 | Produces dense depth cues from single images | Scale ambiguity and model bias | Used for depth estimation stage |
| Neural rendering | NeRF, Gaussian Splatting | Strong visual realism and novel views | Needs good camera poses and training/export structure | Supported through Gaussian export |

The advantage of passive reconstruction is accessibility. A user does not need a dedicated scanner to start the reconstruction process. This matches the motivation of S.P.E.C.T.R.A: ordinary smartphone images can become the starting point for point clouds, meshes, and export-ready 3D assets. The disadvantage is that passive reconstruction depends heavily on capture discipline. Poor lighting, motion blur, reflective surfaces, low texture, and insufficient overlap can reduce reconstruction quality.

## 2.3 Photogrammetry and Smartphone-Based Capture

Photogrammetry estimates measurements and geometry from photographs. Modern digital photogrammetry often uses many overlapping images, detects shared visual features, estimates camera positions, and triangulates 3D points. In a controlled photogrammetry workflow, the operator captures an object or scene from multiple viewpoints with sufficient overlap between consecutive images. The captured image sequence should include rich texture, consistent exposure, minimal blur, and a stable scene.

Smartphones are attractive capture devices because they are affordable, portable, and widely available. They can produce high-resolution images and are already familiar to most users. This makes them suitable for educational reconstruction systems and early-stage prototypes. However, smartphone capture also introduces challenges. The camera may apply automatic exposure, automatic focus, sharpening, noise reduction, rolling shutter effects, and compression. The user may move too quickly, creating blur. Wide-angle lenses may introduce distortion. Some phones save HEIC/HEIF images, which require additional decoding support. S.P.E.C.T.R.A accounts for some of these realities through image filtering, optional HEIC support, keyframe preparation, and camera calibration files.

The capture requirements found in photogrammetry literature directly justify the first two stages of the S.P.E.C.T.R.A pipeline. The raw images are not passed immediately into reconstruction. They are first filtered for feature richness, diversity, and quality. This is important because the failure of early image selection can propagate through the rest of the pipeline. If the selected frames are blurry, repetitive, or textureless, then feature matching becomes unreliable, pose recovery becomes unstable, depth fusion becomes misaligned, and the final mesh becomes noisy.

## 2.4 Structure from Motion

Structure from Motion is a central method in image-based 3D reconstruction. It estimates both the structure of the scene and the motion or position of the camera from multiple images. In a typical SfM pipeline, the system detects image features, matches them between images, estimates epipolar geometry, recovers relative camera poses, triangulates 3D points, and optionally refines the result through bundle adjustment. Schonberger and Frahm's work on COLMAP shows the importance of robust feature matching, track management, triangulation, and optimization in mature SfM systems.

S.P.E.C.T.R.A implements a simplified SfM-style stage in `src/sfm/sparse_recon_live.py`. The script reads prepared keyframes, extracts ORB features, matches descriptors, filters matches, estimates the essential matrix, applies RANSAC, recovers camera pose, triangulates sparse points, and saves camera pose records. The output includes `data/results/sparse_cloud.ply` and `data/results/camera_poses.json`. These outputs are important because they provide the geometric frame used later by dense fusion and Gaussian export.

The essential matrix represents the epipolar relationship between two calibrated camera views. If corresponding feature points are known in two images, the essential matrix can be estimated and decomposed to recover relative rotation and translation direction. RANSAC is used because feature matches may include outliers. A naive pose estimate using all matches would be unstable if many correspondences are incorrect. RANSAC repeatedly samples possible match subsets, estimates a model, and keeps the model that explains the largest set of inliers.

The main strength of SfM is that it provides geometric consistency. It does not merely guess dense structure from image appearance; it uses repeated visual evidence across views. Its main limitation is that it depends on sufficient feature correspondences. Textureless walls, reflective surfaces, transparent objects, strong motion blur, repeated patterns, and poor overlap can reduce the number or reliability of matches. S.P.E.C.T.R.A therefore combines sparse SfM with learned depth estimation, so that the system has both camera pose evidence and dense depth cues.
...
Feature detection identifies repeatable image locations, such as corners, textured patches, and visually distinctive points. Feature descriptors encode the local appearance around those points so that the same scene point can be matched across different images. SIFT and SURF are historically important feature methods, but ORB is popular in practical systems because it is efficient and uses binary descriptors. Rublee et al. introduced ORB as an efficient alternative to SIFT and SURF, combining oriented FAST keypoints with rotated BRIEF descriptors.

S.P.E.C.T.R.A uses ORB in two important places. First, `src/utils/smart_frame_selector.py` counts ORB features to reject frames that are likely to be blurry or textureless. A frame with few ORB keypoints provides weak evidence for SfM. Second, `src/sfm/sparse_recon_live.py` uses ORB features and descriptor matching for pose recovery and sparse triangulation. The same idea therefore supports both dataset preparation and reconstruction.

The benefit of ORB for this project is speed. A student prototype running on a laptop must avoid unnecessarily expensive operations. ORB descriptors can be matched efficiently using Hamming distance, making them suitable for a Python and OpenCV workflow. ORB is also appropriate for a dashboard-driven system where stages should complete in a reasonable time for demonstration.

The limitation is that ORB is not always as distinctive as heavier descriptors. It can struggle under severe viewpoint change, extreme lighting variation, motion blur, repeated texture, or low-detail surfaces. For that reason, S.P.E.C.T.R.A's frame selector uses multiple indicators, not ORB alone. It combines ORB feature count, perceptual hash distance, histogram difference, and sharpness-based scoring to improve frame quality before reconstruction begins.

## 2.6 Camera Calibration and Intrinsic Parameters

Camera calibration estimates the internal parameters of a camera. These include focal length, principal point, and lens distortion coefficients. In pinhole camera geometry, a 3D point is projected into image coordinates through the camera intrinsic matrix. If this matrix is inaccurate, back-projection and pose estimation can produce distorted geometry. Distortion coefficients are also important because smartphone lenses may introduce radial or tangential distortion, especially near image edges.

S.P.E.C.T.R.A includes calibration resources in `data/calibration` and camera intrinsics JSON files in `data`. The project code loads calibration information where available and falls back to estimated intrinsics if an exact calibration match is not found. This is a practical design decision for a prototype, but it also introduces a limitation. Accurate reconstruction should use calibration that matches the exact camera, image resolution, zoom level, and capture mode used during the dataset collection.

Checkerboard calibration is a common method because the known geometry of the checkerboard gives the system reliable 2D-3D correspondences. OpenCV provides standard calibration and undistortion tools. In S.P.E.C.T.R.A, calibration affects both sparse reconstruction and dense fusion. Sparse reconstruction uses calibration when estimating and undistorting image points. Dense fusion uses camera intrinsics to convert depth pixels into 3D points. Gaussian export also uses calibration when writing COLMAP-style camera files.
...
A point cloud is a set of 3D points that represent sampled geometry from a scene or object. Sparse point clouds usually come from triangulated feature matches. Dense point clouds may come from multi-view stereo, depth maps, LiDAR, structured light, or learned depth predictions. S.P.E.C.T.R.A produces multiple point cloud outputs so that reconstruction progress can be inspected at different stages.

The dense fusion stage in `src/fusion/depth_fusion.py` reads depth maps, camera poses, camera calibration, and keyframe images. It back-projects valid depth pixels into 3D camera coordinates using the camera intrinsic matrix. It then transforms those points into a shared world coordinate system using the recovered camera poses. Color information from the source image is attached to the generated points. The result is saved as a pose-aligned dense cloud.

Fusion is important because a single image depth map only describes geometry visible from one viewpoint. Multi-view fusion combines partial views into a more complete representation. However, fusion is also a major source of error. If camera poses are wrong, depth maps are inconsistent, calibration is inaccurate, or scale changes between frames, the fused cloud may become smeared, duplicated, or distorted. S.P.E.C.T.R.A includes thresholding, frame selection, depth clipping, and optional alignment behavior to reduce these problems.

| Representation | Generated From | Strength | Limitation | S.P.E.C.T.R.A Output |
|---|---|---|---|---|
| Sparse point cloud | Feature matches and triangulation | Geometrically meaningful evidence | Low density | `sparse_cloud.ply` |
| Dense point cloud | Depth maps and camera poses | Better scene coverage | Sensitive to depth and pose errors | `dense_fused_cloud_pose_aligned.ply` |
| Cleaned point cloud | Clustering and outlier filtering | Reduced noise | May remove valid isolated details | `dense_fused_cloud_clean.ply` |
| Mesh | Surface reconstruction from points | Editable surface representation | Can fill gaps unrealistically | `mesh_poisson.ply`, `mesh_poisson.obj` |
| Gaussian scene | Images, poses, sparse points | Supports neural rendering workflow | Requires training outside the dashboard | `data/gaussian_splatting_scene` |
...
A point cloud is useful for inspection, but many downstream applications require a surface mesh. A mesh represents geometry using vertices, edges, and faces. The mesh can be exported to 3D tools, edited, rendered, or used in simulation. S.P.E.C.T.R.A uses Poisson surface reconstruction through Open3D in `src/mesh/mesh_surface.py`.

Poisson reconstruction estimates a continuous surface from oriented point samples. Before running Poisson reconstruction, the system estimates normals for the point cloud. Normals describe the local surface direction. If normals are inconsistent, the resulting mesh may be inverted or unstable. After reconstruction, low-density vertices are trimmed because Poisson methods may fill unsupported regions and create smooth surfaces around areas where input points are sparse.

The mesh stage writes both PLY and OBJ files. PLY is useful for point clouds and meshes with vertex attributes. OBJ is widely supported by 3D modeling tools and game engines. The output files are `data/results/mesh_poisson.ply` and `data/results/mesh_poisson.obj`.

The main strength of Poisson meshing is that it can create smooth surfaces from noisy point clouds. Its limitation is that it may hallucinate or close surfaces where data is missing. This matters in S.P.E.C.T.R.A because smartphone captures may not cover every side of an object or room. A mesh can look complete even when parts are inferred rather than measured. The report must therefore discuss mesh results as reconstructed approximations, not as guaranteed ground truth.
...
The conceptual framework shows that S.P.E.C.T.R.A is built on two forms of image evidence. The first is feature evidence, which supports camera pose recovery and sparse geometry through SfM. The second is depth evidence, which supports dense reconstruction through monocular depth estimation. These two evidence streams meet in the fusion stage, where depth maps are placed into a shared coordinate system using recovered poses. The cleaned point cloud then becomes the basis for mesh generation, export, visualization, and evaluation.
